                        ...
                        {(() => {
                          const earned = Number(
                            txn.adminEarning ??
                              ((txn.userPaid || 0) -
                                (txn.sellerEarning || 0) -
                                (txn.deliveryEarning || 0) -
                                (txn.paymentBreakdown?.gst || txn.paymentBreakdown?.tax || 0))
                          );
                          return (
                            <span className={`text-sm font-bold ${earned < 0 ? 'text-rose-600' : 'text-purple-600'}`}>
                              {earned < 0 ? `-₹${Math.abs(earned).toFixed(2)}` : `₹${earned.toFixed(2)}`}
                            </span>
                          );
                        })()}
                      </td>
