  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (step2.profileImage instanceof File) {
        profileImageData = await handleUpload(step2.profileImage, "superfast/restaurant/profile")
      ...
      for (const file of step2.menuImages.filter(f => f instanceof File)) {
        const uploaded = await handleUpload(file, "superfast/restaurant/menu")
      ...
      if (step3.panImage instanceof File) {
        panImageData = await handleUpload(step3.panImage, "superfast/restaurant/pan")
      ...
      if (step3.gstRegistered && step3.gstImage) {
        if (step3.gstImage instanceof File) {
          gstImageData = await handleUpload(step3.gstImage, "superfast/restaurant/gst")
      ...
      if (step3.fssaiImage instanceof File) {
        fssaiImageData = await handleUpload(step3.fssaiImage, "superfast/restaurant/fssai")
            ...
            {daysOfWeek.map((day) => {
              ...
              return (
                <button
                  ...
                  className={`aspect-square flex items-center justify-center rounded-md text-[11px] font-medium ${active ? "bg-[#008226] text-white" : "bg-gray-100 text-gray-800"}`}
                >
                  {day.charAt(0)}
                </button>
              )
