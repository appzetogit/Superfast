  ...
  const fetchRefundData = async () => {
    try {
      ...
      if (response?.data?.success) {
        ...
        setRefundData({
          title: dataObj.title || 'Refund Policy',
          content: textContent
        })
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.put(
        ...
        { title: refundData.title || 'Refund Policy', content: htmlContent },
        { contextModule: "admin" }
      )
      if (response?.data?.success) {
    ...
    <div className="h-full overflow-y-auto bg-slate-50 p-4 lg:p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-600 mt-1">Manage your Refund Policy content</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
          ...
          {viewMode === "edit" ? (
            <Textarea
              ...
              onChange={(e) => setRefundData(prev => ({ ...prev, content: e.target.value }))}
            ...
            <div className="min-h-[600px] w-full rounded-md border border-slate-200 bg-white p-4">
              <div
                className="prose prose-slate max-w-none
                  ...
                  prose-li:my-1
                  leading-relaxed"
                dangerouslySetInnerHTML={{ __html: plainTextToLegalHtml(refundData.content) }}
              />
            </div>
          )}
        </div>
