...
import { ArrowLeft, Clock, MapPin, Heart, Star, Cake } from "lucide-react"
...
export default function CustomCakeRestaurants() {
  ...
  useEffect(() => {
    ...
    const fetchBakeries = async () => {
      try {
        setLoading(true)
        const params = { limit: 300, businessType: "home_bakery", customOrdersEnabled: true, _ts: Date.now() }
        if (zoneId) {
    ...
    <AnimatedPage className="min-h-screen bg-gradient-to-b from-pink-50/20 dark:from-[#0a0a0a] via-white dark:via-[#0a0a0a] to-amber-50/10 dark:to-[#0a0a0a]">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 xl:px-12 pt-4 pb-24 md:py-8 lg:py-10 space-y-4 sm:space-y-6 lg:space-y-8">
        <ScrollReveal>
          <div className="flex items-center gap-3 sm:gap-4 lg:gap-5 mb-4 lg:mb-6">
            ...
            <TextReveal className="flex items-center gap-2 sm:gap-3 lg:gap-4">
              <h1 className="text-lg sm:text-xl md:text-2xl lg:text-3xl xl:text-4xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <Cake className="h-6 w-6 sm:h-8 sm:w-8 lg:h-10 lg:w-10 text-rose-500" />
                Custom Cake Orders
              </h1>
            </TextReveal>
          </div>
        ...
        {showSkeleton ? (
          ...
          <div className="py-16 text-center text-sm text-gray-500">No custom cake restaurants available in your zone right now.</div>
        ) : (
