        ...
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          ...
          <div className="flex-1 relative">
            ...
            <Input
              ...
              className="pl-10 pr-10 h-11 bg-slate-100 dark:bg-zinc-800 border-none focus:ring-2 focus:ring-[var(--primary-theme)] rounded-xl"
            />
            {query && (
            ...
            <button
              ...
              className={`absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded-full transition-all ${isListening ? 'text-[var(--primary-theme)] scale-125 animate-pulse' : 'text-slate-400'}`}
            >
      ...
      <div className="max-w-3xl mx-auto p-4">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-4">
              {categories.map((cat) => (
                ...
                >
                  <div className={`w-14 h-14 rounded-2xl mb-2 flex items-center justify-center overflow-hidden border-2 transition-all ${selectedCategoryId === cat._id ? 'border-[var(--primary-theme)] shadow-lg shadow-[var(--primary-theme)]/10' : 'border-transparent bg-white dark:bg-zinc-900'}`}>
                    {cat.image ? (
                      <img
        ...
        <AnimatePresence>
          {loading && (
            ...
            >
              <Loader2 className="w-8 h-8 text-[var(--primary-theme)] animate-spin mb-3" />
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                ...
                <div className="flex items-center gap-3 overflow-x-auto pb-2 scrollbar-none">
                  {results.categories.map((cat) => (
                    <button
                      key={cat._id}
                      onClick={() => handleCategoryClick(cat._id)}
                      className={`flex items-center gap-3 px-4 py-2.5 rounded-2xl border transition-all flex-shrink-0 ${selectedCategoryId === cat._id
                  ...
                  {results.restaurants.map((r) => (
                    <Link to={`/food/user/restaurants/${r.slug || r._id}`} key={r._id} className="block group">
                      <div className="relative rounded-3xl overflow-hidden aspect-[16/9] mb-3 bg-slate-200">
                        ...
                        {r.offer && (
                          <div className="absolute top-4 left-0 bg-[var(--primary-theme)] text-white text-[10px] font-black px-3 py-1.5 rounded-r-lg shadow-lg flex items-center gap-1 tracking-tighter">
                      ...
                      <div className="flex items-center justify-between px-1">
                        ...
                        <div className="text-[10px] font-bold text-[var(--primary-theme)] bg-[#fff5f5] dark:bg-[var(--primary-theme)]/10 px-2 py-0.5 rounded-full uppercase tracking-wider">
                          Top Pick
                        </div>
                      </div>
                    </Link>
            ...
            {!loading && (!results.restaurants || results.restaurants.length === 0) && (!results.dishes || results.dishes.length === 0) && (!results.categories || results.categories.length === 0) && (
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <Button variant="outline" onClick={handleClear} className="mt-6 rounded-xl border-[var(--primary-theme)] text-[var(--primary-theme)] hover:bg-[#fff5f5]">
                  Clear all filters
                </Button>
              </div>
            )}
