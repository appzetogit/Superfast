  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      try {
        ...
        const effectiveZone = zoneId || (typeof window !== 'undefined' ? window.localStorage.getItem('userZoneId') : null);
        if (effectiveZone) params.zoneId = effectiveZone;
  ...
  return (
    ...
    >
      ...
      <h2 className="text-[11px] font-extrabold text-slate-900 dark:text-white tracking-widest uppercase mb-3 px-4">
        {favouriteCategories.length > 0 ? 'WHAT\'S ON YOUR MIND?' : 'RECOMMENDED FOR YOU'}
      </h2>
