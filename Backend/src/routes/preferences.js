...
router.get('/recommendations', authMiddleware, async (req, res, next) => {
    try {
        ...
        // ZONE FALLBACK: If no recommendations found yet (user hasn't set preferences OR no match),
        // return top restaurants in the active zone. This ensures zone-based data always shows.
        ...
        // GLOBAL FALLBACK: If still no restaurants (no zoneId provided), return top-rated approved restaurants
        if (recommendedRestaurants.length === 0) {
            recommendedRestaurants = await FoodRestaurant.find({ status: 'approved' })
                .sort({ rating: -1, totalRatings: -1 })
                .limit(12)
                .lean();
        }
