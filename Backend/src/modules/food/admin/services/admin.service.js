...
export async function getFoods(query) {
    ...
    const foods = list.map((f) => ({
        ...
        basePrice: f.price,
        price: f.price !== undefined && f.price !== null && Number(f.price) >= 0 ? f.price : getFoodDisplayPrice(f),
...
const getAdminFoodCreatePricing = (body = {}) => {
    ...
    if (!Number.isFinite(sentPrice) || sentPrice <= 0) throw new ValidationError('Price must be greater than 0');
    return {
        price: sentPrice,
        variants: []
    };
        ...
        try {
            ...
            await notifyOwnersSafely(
                [{ ownerType: 'RESTAURANT', ownerId: updated._id }],
                {
                    ...
                    image: updated.profileImage || 'https://i.ibb.co/3m2Yh7r/SUPERFAST-Brand-Image.png',
                    data: {
