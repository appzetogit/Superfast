...
export const listApprovedRestaurants = async (query = {}) => {
    ...
    // Extract activeZoneId at function scope so it's accessible throughout
    ...
    // Bypass if Developer Mode is enabled OR if zoneId is provided (zone filter is sufficient).
    const isDevGeoBypass = Boolean(devFilter?.isDevMode);
    const isZoneGeoBypass = Boolean(activeZoneId); // Zone filter already scopes restaurants; skip geo radius
    const wantsGeo = !isDevGeoBypass && !isZoneGeoBypass && ((radiusKm !== null) || sortBy === 'nearest');
